...
export function SchedulingEmbed({ lead, eventUrl, allowMock }: Props) {
  ...
  const calendlyUrl = useMemo(() => {
    ...
    url.searchParams.set('a1', lead.id);
    url.searchParams.set('a2', lead.phone);
    url.searchParams.set('utm_source', 'paintleadsai');
    url.searchParams.set('utm_medium', 'self_scheduling');
  ...
  async function bookMock(label: string) {
    ...
    const res = await fetch('/api/appointments', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ leadId: lead.id, scheduledAt, timezone: Intl.DateTimeFormat().resolvedOptions().timeZone, externalBookingId: `mock-${lead.id}-${Date.now()}`, externalEventUri: `mock:event:${lead.id}`, externalInviteeUri: `mock:invitee:${lead.id}:${Date.now()}`, meetingLocation: label, provider: 'mock' }) });
  ...
  return <div className="card overflow-hidden p-0">
    ...
    {calendlyUrl ? <><div className="calendly-inline-widget min-h-[720px] w-full" data-url={calendlyUrl} /><Script src="https://assets.calendly.com/assets/external/widget.js" strategy="afterInteractive" onLoad={() => setLoading(false)} onError={() => { setLoading(false); setError('The Calendly calendar could not load. Please refresh or contact us for help.'); }} /></> : <div className="p-6">
      <p className="font-bold text-brand">Development mock calendar</p><p className="mt-2 text-slate-600">Calendly is not configured. These sample times are available only outside production.</p>
